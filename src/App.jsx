...
export default function App() {
  ...
  const load = useCallback(async (q, t, pg) => {
    ...
    try {
      ...
      // ✅ Fix: use ? or & depending on whether endpoint already has query params
      const separator = endpoint.includes('?') ? '&' : '?'
      const res = await fetch(
        `https://api.themoviedb.org/3/${endpoint}${separator}api_key=${API_KEY}&page=${pg}`
      )
              ...
              {!loading && !error && movies.length > 0 && (
                <>
                  ...
                  <Pagination
                    page={page}
                    total={total}
                    onPrev={() => { setPage(p => p - 1); window.scrollTo({ top: 0, behavior: 'smooth' }) }}
